...
double Array::SummaNegativeNumber() const
{
	double SummaNegativeNumber = 0;
	for (int i = 0; i < size(); i++)
		if (elems[i] < 0)
		{
			SummaNegativeNumber += elems[i];
		}

	return SummaNegativeNumber;
}
void Array::myTask()
{
	double SummaNegativeNumber = this->SummaNegativeNumber();

	for (int i = 0; i < size(); i++)
	{
		elems[i] += (SummaNegativeNumber / 2.);
	}
}
